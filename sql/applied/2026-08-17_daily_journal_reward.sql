...
BEGIN
  ...
  -- 오늘(KST) 지급 시도. UNIQUE 인덱스가 중복을 막으므로
  -- ON CONFLICT DO NOTHING 이면 이미 받은 경우 아무 행도 안 들어간다.
  INSERT INTO public.mobil_grants (profile_id, grant_type, amount, note)
  VALUES (v_profile, 'daily_journal', v_amount, '일일 일지 최초 작성 보상')
  ON CONFLICT (profile_id, grant_date)
    WHERE (grant_type = 'daily_journal'::text)
  DO NOTHING;

  GET DIAGNOSTICS v_inserted = ROW_COUNT;  -- 1이면 지급, 0이면 이미 받음
