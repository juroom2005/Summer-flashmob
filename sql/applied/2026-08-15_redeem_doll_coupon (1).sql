-- ═══════════════════════════════════════════════════════════════════
-- ★ redeem_doll_coupon() 정본 — 여기만 수정할 것 ★
--   · 인형 교환에 필요한 교환권 장수 = v_required (현재 10).
--     ⚠ 되돌아간 이력 있음: 과거 1장으로 교환되던 버그. 반드시 10 유지.
--   · 프론트 InventorySection.tsx 의 COUPON_REQUIRED 와 값을 일치시킬 것.
--   · 이 함수는 이 파일이 유일 정본. 다른 데서 CREATE OR REPLACE 하지 말 것.
-- ═══════════════════════════════════════════════════════════════════
...
DECLARE
  ...
  v_required   integer := 10;   -- 인형 1개 교환에 필요한 쿠폰 장수
  v_total      integer;
  v_to_remove  integer;
  v_rec        record;
...
BEGIN
  ...
  -- 10장 미만이면 교환 불가. (에러명은 기존 호환 위해 no_coupon 유지)
  ...
  -- 쿠폰 v_required(10)개 차감 (가장 오래된 스택부터, 빈 행 삭제)
  v_to_remove := v_required;
  FOR v_rec IN
    SELECT id, quantity
      FROM public.inventory_items
     WHERE profile_id = v_profile_id
  ...
  RETURN jsonb_build_object(
    ...
    'remaining_coupons', v_total - v_required
  );
END;
$function$;
