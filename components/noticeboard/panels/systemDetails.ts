...
export const SYSTEM_DETAILS: Record<string, SystemDetail> = {
  stat: {
    ...
    blocks: [
      {
        type: "p",
        text: "총 세 가지 스탯이 존재하며, 러닝 중 아르바이트나 연습을 통해 키울 수 있습니다. 일정 경험치 달성 시 스탯을 추가합니다. 스탯별 수준은 다음과 같습니다. 스탯은 엔딩 시 캐릭터에 영향을 미칩니다.",
      },
      {
        type: "grade",
  ...
  badge: {
    ...
    blocks: [
      {
        type: "p",
        text: "각 스탯이 커뮤니티 최종 레벨인 5레벨에 도달할 시 획득 가능한 리워드입니다. 퍼스나콘의 형태로, 홈페이지 활동 시 확인이 가능하며 선착순 3인에 한하여 메달 컬러를 추가로 받습니다. 여러개의 뱃지를 소유한 경우 마이페이지에서 변경이 가능합니다. 뱃지 이미지는 아래와 같습니다.",
      },
    ],
    iconGroups: [
      {
  ...
  daily: {
    ...
    blocks: [
      ...
      {
        type: "p",
        text: "캐릭터는 하루에 3회 아르바이트나 연습 활동을 할 수 있으며, KST AM 12:00가 지나면 캐릭터의 행동 횟수가 회복됩니다. 커뮤 전용 홈페이지 메뉴에서 플레이 가능하며, 한 번 행동할 때마다 행동 횟수가 1회씩 차감됩니다. 잔여 행동 횟수는 다음날로 이월되지 않고 초기화됩니다. 아래 아이콘을 터치하여 각 활동의 가이드를 확인 할 수 있습니다.",
      },
    ],
    iconGroups: [
      {
  ...
  log: {
    ...
    blocks: [
      {
        type: "p",
        text: "모두가 작성할 수 있는 다이어리입니다. 모두가 공유하는 연습 일지 노트라는 컨셉입니다. 커뮤 전용 홈페이지 위젯에 위치해 있으며 마우스 클릭 시 팝업됩니다. 타이핑, 드로잉, 이미지 첨부, 스티커 부착 등이 가능합니다. 역극에 해당 내용을 인용하는 것이 가능하나, 일지를 작성하지 않는 멤버가 있을 수도 있기에 **적당한 어필 용도**로만 사용 부탁 드립니다.",
      },
      {
        type: "list",
        items: [
  ...
  shop: {
    ...
    blocks: [
      {
        type: "p",
        text: "모브 재단에서 운영하는 제법 큰 규모의 매점으로 여러 가지 물건을 팔고 있습니다. 아르바이트, 이벤트를 통해 얻을 수 있는 '모빌'만을 사용해 물건을 구매할 수 있으며, 가끔 파격적인 세일 이벤트를 진행하기도 합니다. 평이한 상점 이용을 위해 **현재 보유 포인트를 필수 기입** 부탁드립니다. 이를 지키지 않아 생긴 문제는 책임 지지 않습니다.",
      },
      {
        type: "p",
        text: "매점에서 구매한 물건 또는 모빌 자체에 대한 선물이 가능합니다. 00계 태그 외 특별한 양식은 없으며 별도의 확인이나 표시 없이 역극을 이어나가주셔도 괜찮습니다.",
      },
      { type: "bubble", ex: true, text: "너 정말 브레이크 댄스를 잘 추던걸? (300모빌을 건넨다.) @00" },
    ],
  },
};
